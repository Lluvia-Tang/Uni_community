...
- 发送私信  
- 删除私信 
- 在表现层进行统一异常处理/(引入AOP)在访问业务前统一记录访问日志 
- 点赞(Redis) 
- 个人主页->我收到的赞  
- 关注、取消关注  
- 关注列表、粉丝列表  
...
- 发送系统通知(Kafka)  
- 显示系统通知  
- 开发校园集市搜索功能(Elasticsearch) 
- 权限控制  
- 置顶、加精、删除/取消置顶、取消加精(Security)  
- 网站数据统计(HyperLogLog,Bitmap)  
- 热帖排行(Quartz)  
- 生成长图  
- 优化网站的性能(Caffeine)  
- 项目发布-单元测试 
...
- 本地缓存
  ...
  - 不能跨服务器（和用户强关联的数据不推荐存入）
- 分布式缓存
  - 将数据缓存在NoSQL数据库上，能够跨服务器
...
##### 配置Redis集群
- 配置Redis集群，提高Redis服务的可用性（防止服务器挂掉导致缓存雪崩），支持项目大规模的写入并发，或者缓存数据量巨大的情况。
- 首先保证windows系统上搭建好了Redis集群
- 之后在application.yml中对redis集群进行配置
```xml
spring:
  redis:
    database: 11
